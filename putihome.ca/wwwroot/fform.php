...
$query = "SELECT b.id, b.title, b.description, c.city, c.state FROM event_calendar a INNER JOIN puti_sites c ON (a.site = c.id) INNER JOIN (SELECT * FROM puti_agreement_lang WHERE lang = '" . $db->quote($Glang) . "') b ON (a.agreement = b.agreement_id) WHERE a.id = '" . $db->quote($_REQUEST["event_id"]) . "'";
//echo $query;
