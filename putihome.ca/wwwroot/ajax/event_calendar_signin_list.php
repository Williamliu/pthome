...
try {
	...
	while($row0 = $db->fetch($result0)) {
		...
		$evt_id 	= $row0["id"];
		$query1 	= "SELECT * FROM event_calendar WHERE deleted <> 1 AND id = '" . $db->quote($evt_id) . "'";
		...
		$resultlf = $db->query("SELECT logform FROM puti_class WHERE id = '". $db->quote($row1["class_id"]) ."'");
		...
		$query2 = "SELECT * FROM event_calendar_date WHERE deleted <> 1 AND event_id = '" . $db->quote($row1["id"]) . "' ORDER BY event_date";
