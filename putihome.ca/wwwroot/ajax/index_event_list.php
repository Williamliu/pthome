...
try {
	...
	if( $_REQUEST["site"]=="" && $_REQUEST["branch"]=="") {
		  ...
		  $query0 = "SELECT * FROM puti_sites  
					  WHERE status = 1 AND id = '" . $db->quote($_REQUEST["site"]) . "'  
					  ORDER BY sn";
		...
		if($_REQUEST["site"]=="" && $_REQUEST["branch"]=="") {
			$query1 = "SELECT distinct a.* FROM puti_branchs a 
								...
								WHERE a.internal = 0 AND b.deleted <> 1 AND b.status = 2 AND c.deleted <> 1 AND c.status = 1 AND
									  e.site_id = '" . $db->quote($site_id) . "' AND b.site = '" . $db->quote($site_id) . "'  $ccc ORDER BY a.sn";
		} elseif($_REQUEST["branch"]=="") {
			$query1 = "SELECT distinct a.* FROM puti_branchs a INNER JOIN puti_sites_branchs b ON (a.id = b.branch_id) WHERE a.internal = 0 AND b.site_id = '" . $db->quote($site_id) . "' ORDER BY a.sn";
		} else {
			$query1 = "SELECT distinct a.* FROM puti_branchs a INNER JOIN puti_sites_branchs b ON (a.id = b.branch_id) WHERE a.internal = 0 AND b.site_id = '" . $db->quote($site_id) . "' AND a.id = '" . $db->quote($_REQUEST["branch"]) . "' ORDER BY a.sn";
		}
		...
		while( $row1 = $db->fetch($result1) ) {
			...
			$query2 = "SELECT distinct b.id, b.site, b.branch, b.title, b.description, b.start_date, b.end_date, b.status,
								...
								WHERE b.deleted <> 1 AND b.status = 2 AND a.deleted <> 1 AND a.status = 1 AND 
									  b.site = '" . $db->quote($site_id) . "' AND b.branch = '" . $db->quote($branch_id) . "' $ccc  
