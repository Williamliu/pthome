...
try {
	...
	$query0 = "SELECT b.id, b.event_id, b.title, b.description, b.yy, b.mm, b.dd, b.event_date, b.start_time, b.end_time, b.status,
					  ...
					  a.title as event_title, a.description as event_description, a.status as event_status, a.class_id, 
					...
					WHERE d.internal = 0 AND a.deleted <> 1 AND a.status >= 1 AND  
						  b.deleted <> 1 AND b.status = 1 AND 
						  a.site = '" . $db->quote($_REQUEST["site"]) . "' AND
	...
	while($row0 = $db->fetch($result0)) {
		...
		$evt[$cnt]["site_desc"]			= $row0["site_desc"];
		...
		$resultlf 	= $db->query("SELECT logform FROM puti_class WHERE id = '" . $row0["class_id"] . "'");
		$rowlf 		= $db->fetch($resultlf);
		$evt[$cnt]["logform"] 			= $rowlf["logform"];
		$evt[$cnt]["event_id"] 			= $row0["event_id"];
