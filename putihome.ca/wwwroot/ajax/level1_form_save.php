...
try {
	...
	$publicSession  		= $db->quote($_REQUEST["publicSession"]);

	$_REQUEST["password"] 	= trim($db->quote($_REQUEST["password"]));
	$_REQUEST["cpassword"] = trim($db->quote($_REQUEST["cpassword"]));
	...
	if( $member_id > 0 )  {
		  ...
		  $query = "SELECT * FROM puti_members WHERE deleted <> 1 AND email = '" . trim($db->quote($_REQUEST["email"])) ."'";
		  if($db->exists($query)) {
	...
	// update language 
	$db->query("DELETE FROM puti_members_lang WHERE member_id = '" . $db->quote($member_id) . "'");
	...
	$db->query("DELETE FROM puti_members_others WHERE member_id = '" . $db->quote($member_id) . "'");
	$fields = array();
	$fields["member_id"] 					= $member_id;
	...
	$db->query("DELETE FROM puti_members_hearfrom WHERE member_id = '" . $db->quote($member_id) . "'");
	...
	$db->query("DELETE FROM puti_members_symptom WHERE member_id = '" . $db->quote($member_id) . "'");
	...
	$query = "SELECT id, shelf FROM event_calendar_enroll WHERE event_id = '" . $db->quote($event_id) . "' AND member_id = '" . $db->quote($member_id) . "'";
	$result = $db->query( $query );
	if( $db->row_nums($result) > 0 )  {
		...
		if( $shelf <= 0 ) {
			$query_del = "SELECT id, shelf FROM event_calendar_enroll WHERE deleted = 1 AND shelf > 0 AND event_id = '" . $db->quote($event_id) . "' ORDER BY id ASC";
			if( $db->exists( $query_del ) ) {
				$result_del = $db->query( $query_del );
				$row_del = $db->fetch( $result_del );
				$db->query("UPDATE event_calendar_enroll SET shelf = 0 WHERE id = '" . $db->quote($row_del["id"]) . "'");
				$shelf = $row_del["shelf"];
			} else {
				$query_max = "SELECT MAX(shelf) as max_shelf FROM event_calendar_enroll WHERE event_id = '" . $db->quote($event_id) . "'";
		...
		$query = "UPDATE event_calendar_enroll SET group_no = 0, deleted = 0, online = 1, shelf = '" . $db->quote($shelf) . "' WHERE event_id = '" . $db->quote($event_id) . "' AND member_id = '" . $db->quote($member_id) . "'";
		$result = $db->query( $query );
	} else {
		$fields = array();
	...
	//if( $_SESSION[$_SERVER['HTTP_HOST'] . ".pubSessID"] == "" ) {
		...
		$db->query("UPDATE puti_members SET hits = hits + 1, last_login = '". time() ."', sess_exp = '" . (time() + 3600 * 2) . "', sess_id = '" . $db->quote($sess_id) . "' WHERE id = '" . $db->quote($member_id) . "'");
