...
try {
	...
	$query = "SELECT id FROM puti_members WHERE deleted <> 1 AND 
				( 	email = '" . $db->quote($_REQUEST["email"]) . "' OR  
					replace(replace(replace(phone,' ',''),'-',''),'.','') like '%" . str_replace(array(" ","-","."), array("","",""), $db->quote($_REQUEST["phone"])) . "%' OR 
					replace(replace(replace(cell,' ',''),'-',''),'.','') like '%" . str_replace(array(" ","-","."), array("","",""), $db->quote($_REQUEST["cell"])) . "%' 					
				) AND first_name = '" . cTYPE::utrans($db->quote($_REQUEST["first_name"])) . "'";
	...
	if( $db->row_nums($result) > 0 )  {
		  ...
		  $query = "SELECT id FROM event_calendar_enroll WHERE event_id = '" . $db->quote($_REQUEST["event_id"]) . "' AND member_id = '" . $db->quote($member_id) . "'";
		  $result = $db->query( $query );
		  if( $db->row_nums($result) > 0 )  {
			  $query = "UPDATE event_calendar_enroll SET group_no = 0, deleted = 0, online = 1 WHERE event_id = '" . $db->quote($_REQUEST["event_id"]) . "' AND member_id = '" . $db->quote($member_id) . "'";
			  $result = $db->query( $query );
		  } else {
			  $fields = array();
