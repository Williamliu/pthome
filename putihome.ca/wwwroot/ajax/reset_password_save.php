...
try {
	...
	$query 	= "SELECT id FROM puti_members WHERE password_link = '" . $db->quote($_REQUEST["password_link"]) . "' AND password_exp > '" . time() . "'";
	if( $db->exists($query) ) {
		$query = "UPDATE puti_members SET password_link = '', password_exp = 0, password = '" . $db->quote($_REQUEST["password"]) . "', password_hits = password_hits + 1 WHERE password_link = '" . $db->quote($_REQUEST["password_link"]) . "'";
		$db->query($query);
	} else {
		$response["errorCode"] 		= 1;
