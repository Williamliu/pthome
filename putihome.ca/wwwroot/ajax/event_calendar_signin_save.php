...
try {
	...
	$query0 = "SELECT id, first_name, last_name, dharma_name, email, phone, cell FROM puti_members WHERE deleted <> 1 AND status = 1 AND ( 
			  	replace(replace(replace(phone,' ',''),'-',''),'.','') 	= '" . str_replace( array(" ", "-", "."),array("","",""), trim($db->quote($_REQUEST["member"])) ) . "' OR 
			  	replace(replace(replace(cell,' ',''),'-',''),'.','')	= '" . str_replace( array(" ", "-", "."),array("","",""), trim($db->quote($_REQUEST["member"])) ) . "' OR 
				email = '" . trim($db->quote($_REQUEST["member"])) . "' OR
				dharma_name = '" . cTYPE::utrans(trim($db->quote($_REQUEST["member"]))) . "' OR
				concat(first_name , ' ' , last_name) = '" . cTYPE::utrans(trim($db->quote($_REQUEST["member"]))) . "')";
	...
	if( $db->row_nums($result0) > 1 ) {
		...
		$query1 = "SELECT id, group_no FROM event_calendar_enroll WHERE  event_id = '" . $db->quote($event_id) . "' AND member_id = '" . $db->quote($member_id) . "'";
		$result1 = $db->query( $query1 );
		if( $db->row_nums($result1) > 0 )  {
			$query = "UPDATE event_calendar_enroll SET group_no = 0, deleted = 0, online = 1 WHERE deleted = 1 AND event_id = '" . $db->quote($event_id) . "' AND member_id = '" . $db->quote($member_id) . "'";
			$db->query( $query );
