...
if( $_SESSION[$_SERVER['HTTP_HOST'] . ".pubSessID"] == "" ) {
	...
	$query = "SELECT id FROM puti_members WHERE deleted <> 1 AND status =1 AND sess_id = '" . $sess_db->quote($sess_id) . "' AND sess_exp > '" . time() . "'";
	if( $sess_db->exists($query) ) {
		$query = "UPDATE puti_members SET sess_exp = '" . (time() + 3600 * 2) . "' WHERE deleted <> 1 AND status = 1 AND sess_id = '" . $sess_db->quote($sess_id) . "'";
		$sess_db->query( $query );
		$sess_pass = true;
	} else {
		$sess_pass = false;
