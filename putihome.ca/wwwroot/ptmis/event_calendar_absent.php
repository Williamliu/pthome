...
<head>
		...
		$(function(){
			  ...
			  allObj = new LWH.cTABLE({
										  ...
										  url:		"ajax/event_calendar_absent_select.php",
										  cache:	false,
										  expire:	3600,
		...
		function headHTML(obj, others) {
			...
			var html = '';
			html += '<tr>';
			html += '<td colspan="' + (9 + hcnt) + '" align="center"><span style="font-size:12px; font-weight:bold;">' + words["menu_absent"] + '</span></td>';
			html += '</tr>';
		...
		function pageHTML(pgData) {
			...
			for(var idx in pgData.rows) {
				...
				html += '<input class="enroll enroll-trial" type="checkbox" enroll_id="' + eObj.enroll_id + '" value="1" />';
				html += '</td>';
				
				/*
				html += '<td class="tabQuery-table-header">';
				html += '<input class="enroll enroll-unauth" type="checkbox" enroll_id="' + eObj.enroll_id + '" value="1" />';
				html += '</td>';
				*/
				
				html += '<td class="tabQuery-table-header">';
				html += '<input class="enroll enroll-sign" type="checkbox" enroll_id="' + eObj.enroll_id + '" value="1" />';
				html += '</td>';
				...
				for(var idx1 in allObj.others) {
					...
					for(var i=1; i<=allObj.others[idx1].checkin; i++) {
						...
						html += '<span class="enroll day-attend" event_id="' +allObj.others[idx1].event_id + '" event_date_id="' + allObj.others[idx1].event_date_id + '" enroll_id="' + eObj.enroll_id + '" value="' + i + '">';
						html += '<input class="enroll day-attend" type="checkbox" event_id="' +allObj.others[idx1].event_id + '" event_date_id="' + allObj.others[idx1].event_date_id + '" enroll_id="' + eObj.enroll_id + '" value="' + i + '" />';
						html += '</span>';
						html += '</td>';
					}
		...
		function doneHTML(pgData) {
			for(var idx in pgData.rows) {
				...
				for(var idx1 in pgData.rows[idx].dates) {
					...
					var status = '<input class="enroll day-attend" type="checkbox" event_id="' + eid + '" event_date_id="' + cid + '" enroll_id="' + rid + '" value="' + sn + '" />';
					//if(st==0) status = '<input class="enroll day-attend" type="checkbox" event_id="' + eid + '" event_date_id="' + cid + '" enroll_id="' + rid + '" value="' + sn + '" />';
					...
					if(st==8) {
						$("input.day-attend[event_id='" + eid + "'][enroll_id='" + rid + "'][event_date_id='" + cid + "'][value='" + sn + "']").attr("checked",true);
					}
				}
			}
		}
		...
		function toJSON() {
			var eObj = [];
			$("input.enroll-sign[enroll_id]").each(function(idx1, el1) {
		...
		function save_ajax() {
				  ...
				  $.ajax({
					  ...
					  error: function(xhr, tStatus, errorTh ) {
				  		  $("#wait").loadHide();
						  alert("Error (event_calendar_absent_save.php): " + xhr.responseText + "\nStatus: " + tStatus);
					  },
					  ...
					  type: "post",
					  url: "ajax/event_calendar_absent_save.php"
				  });
		}
