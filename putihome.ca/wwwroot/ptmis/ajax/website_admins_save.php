...
try {
	...
	if($_REQUEST["admin_id"] < 0) {
		if(trim($_REQUEST["email"]) != "" ) {
			$query = "SELECT id FROM website_admins WHERE deleted <> 1 AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["user_name"])) . "' OR email = '" . trim($_REQUEST["email"]) . "')";
		} else {
			$query = "SELECT id FROM website_admins WHERE deleted <> 1 AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["user_name"])) . "')";
		}
		...
		if( trim($_REQUEST["email"]) != "" ) {
			$query = "SELECT id FROM website_admins WHERE deleted <> 1 AND id <> '" . $_REQUEST["admin_id"] . "' AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["user_name"])) . "' OR email = '" . trim($_REQUEST["email"]) . "')";
		} else {
			$query = "SELECT id FROM website_admins WHERE deleted <> 1 AND id <> '" . $_REQUEST["admin_id"] . "' AND  user_name = '" . cTYPE::utrans(trim($_REQUEST["user_name"])) . "'";
		}
		$result = $db->query( $query );
		if( $db->row_nums($result) > 0 )  {
