...
try {
	...
	$query_base = "SELECT 	a.event_id, a.group_no, COUNT(a.id) as enroll,
							SUM(trial) as trial,  SUM(unauth) as unauth, SUM(signin) as signin, SUM(a.online) as online,  
	...
	$html.= '<tr>';
	$html.= '<td colspan="' . (9 + $hcnt) . '" align="center" height="40" style="font-size:12px; border:0px; font-weight:bold;">' . $title . '</td>';
	$html.= '</tr>';


	$html .= '<tr>';
	$html .= '<td colspan="' . (9 + $hcnt) . '" align="center"><span style="font-size:12px; font-weight:bold;">' . $words["event attendance"] . '</span></td>';
	$html .= '</tr>';
