...
/*********************  single student form ************************************/
			  /*
			  if( file_exists($CFG["upload_path"] . "/small/" . $row["member_id"] . ".jpg") ) 
				  $photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $_REQUEST["member_id"] . '" width="120" height="160" style="position:absolute; top:5px; right:5px; border:1px solid #cccccc; z-index:999;" />';
			  else 
				  $photo_html = '';
		  	  */
		  
			  $width_one 	= 'width="100" height="25" style="font-size:16px;white-space:nowrap;"';
			  $width_two 	= 'width="200" height="25" style="text-align:left;"';
			...
			$html .= '<table border="1" cellpadding="0" cellspacing="0" style="font-size:16px; position:aboslute;" width="650">';
			  ...
			  $html.= '<td valign="top" width="50%">';
				  ...
				  $html.= '<table border="0" cellpadding="2" style="font-size:16px;" width="100%">';
				  $html.= '<tr>
							  <td colspan="2"><span style="font-size:18px;">' . $words["personal information"] . ' </span></td>
						   </tr>';
		...
		if( $admin_user["lang"] != "en" ) {
				  ...
				  $html.= '<tr><td ' . $width_one . '>' . $words["gender"] . ' *</td>
							   <td ' . $width_two . '>
								  <input type="radio" id="gender_male" name="gender' . $row_base["member_id"] . '"  value="Male" '. ($row_base["gender"]=="Male"?'checked="true"':'') .' />'. $words["male"] .'  
								  <input type="radio" id="gender_female" name="gender' . $row_base["member_id"] . '"  style="margin-left:10px;" value="Female" '. ($row_base["gender"]=="Female"?'checked="true"':'') .' />' . $words["female"] . '
							   </td>
						  </tr>';
				  $html.= '<tr><td ' . $width_one . ' valign="top">' . $words["age range"] . ' *</td>
							   <td ' . $width_two . '>' . cHTML::radio( ("age_range" . $row_base["member_id"]) ,$ages, $row_base["age"], 3) .'</td>
						  </tr>';
		} else {
				  $html.= '<tr><td ' . $width_one . '>' . $words["first name"] . ' *</td>
							   <td ' . $width_two . '>' .  cTYPE::gstr($row_base["first_name"]) .'</td>
				  ...
				  $html.= '<tr><td ' . $width_one . ' valign="top">' . $words["age range"] . ' *</td>
                    ...
                    while($row_lang = $db->fetch($result_lang) ) {
	                    $mem_langs[] = $row_lang["language_id"];
                    }
	
    	  
				  $html.= '<tr>
							  <td colspan="2" ' . $line_top. '><span style="font-size:18px;">' . $words["language ability"] . '</span></td>
						   </tr>
						  <tr>
							  <td valign="top">' . $words["preferred language"] . ' <span class="required">*</span>: </td>
								  <td colspan="3" align="left">' .
										  cHTML::radio( ("member_lang" . $row_base["member_id"]) ,$rows_langs, $row_base["language"], 2)
								  . '</td>
							  </tr>
							  <tr>
								  <td valign="top">' . $words["language ability"] . ': </td>
								  <td colspan="3" align="left">' .
										  cHTML::checkbox( ("language" . $row_base["member_id"]) ,$rows_langs, 2 , $mem_langs)
								  . '</td>
							  </tr>';
		  
		  
				  $html.= '<tr>
							  <td colspan="2" ' . $line_top. '><span style="font-size:18px;">' . $words["address information"] . '</span></td>
						   </tr>';
			  ...
			  $html.= '<td valign="top" ' . $width_colspan . '  width="50%">';
				  $html.= '<table border="0" cellpadding="1" style="font-size:16px;" width="100%">';
					...
					$html.= '<tr>
								<td colspan="2"><span style="font-size:18px;">' . $words["contact information"] . '</span></td>
							 </tr>';
				  ...
				  $html.= '<tr>
							  <td colspan="2" ' . $line_top. ' ' . $width_colspan . '><span style="font-size:18px;">' . $words["emergency contact name and relationship"] . '</span></td>
						   </tr>';
				  ...
				  $html.= '<tr>
							  <td colspan="2" ' . $line_top. ' ' . $width_colspan . '><span style="font-size:18px;">' . $words["how did you hear about us?"] . '</span> *</td>
						   </tr>';
				  ...
				  $html.= '<tr>
							  <td colspan="2" ' . $line_top. ' ' . $width_colspan . '><span style="font-size:18px;">' . $words["ailment & symptom"] . '</span></td>
						   </tr>';
			  ...
			  $html.= '<tr>
						  <td colspan="2"  style="border-top:0px; border-bottom:0px;"><br>
							  ...
							  <center>
								  ...
								  <input type="checkbox" id="iagree" ' . ($row_base["email_flag"]?"":"") . '	name="email_flag' . $row_base["member_id"] . '" value="1" style="margin-left:50px;" /><b>' . $words["i agree"] . '</b>
							  </center><br>
						  </td>
					   </tr>';
