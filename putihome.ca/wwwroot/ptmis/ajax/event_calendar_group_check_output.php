...
try {
	...
	$query = "SELECT  a.id as enroll_id, a.created_time as enroll_date, a.confirm, a.group_no, a.trial, a.shelf, a.unauth, a.attend, a.paid, a.paid_date, a.amt, a.invoice, 
					  b.id, b.id as member_id, b.first_name, b.last_name, b.dharma_name, b.dharma_pinyin, b.alias, b.legal_first, b.legal_last, b.language, b.age, b.birth_yy, b.member_yy, b.member_mm, b.member_dd, b.memo, b.gender, b.email, b.phone, b.cell, b.city, b.site,
	...
	while( $row = $db->fetch($result)) {
		if( $old_val != $row["group_no"] ) {
			...
			$query_lead = "SELECT a.id as enroll_id, a.confirm, a.trial, a.unauth, a.attend, a.shelf,  
								 b.id, b.first_name, b.last_name, b.dharma_name, b.alias 
								FROM event_calendar_enroll a 
								INNER JOIN puti_members b ON (a.member_id = b.id)   
								INNER JOIN event_calendar c ON (a.event_id = c.id) 
								WHERE  a.deleted <> 1 AND 
			...
			$gcnt = 0;
			$html.= '<tr>';
			$html.= '<td colspan="35" align="center" height="40" style="font-size:18px; border:0px; font-weight:bold;">' . cTYPE::gstr($row["title"]) . ' [ ' . date("M d, Y", $row["start_date"]) . ($row["start_date"]>0?' ~ ' .date("M d, Y", $row["end_date"]):'') .  ' ]</td>';
			$html.= '</tr>';

			$html.= '<tr>';
			$html.= '<td colspan="35" align="left" valign="middle" height="40" style="font-size:18px; font-weight:bold;">';
		...
		//$html.= '<td ' . $width_two . ' align="center">' . ($row["birth_yy"]>0?$row["birth_yy"]:"") . '</td>';
		...
		$row9	    = $db->fetch($result9);

        $html.= '<td ' . $width_two . ' align="center">' . ($row9["count"]>0?'':'Y') . '</td>';
        /* end of new people */
		...
		$html.= '<td ' . $width_two . '>' . $langs[$row["language"]] . '</td>';
	...
	//$html.= '</tr>';
	$html.= '<tr>';
	$html.= '<td colspan="34" style="font-size:18px; font-weight:bold;">' . cTYPE::gstr($words["total"]) . ': ' . $cnt . '</td>';
	$html.= '</tr>';
