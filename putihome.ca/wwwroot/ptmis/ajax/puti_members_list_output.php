...
try {
	...
	*/
	...
	while( $row = $db->fetch($result)) {
		...
		// language ability
		$query_lang 	= "SELECT 	b.title 
								FROM puti_members_lang a 
								INNER JOIN puti_info_language b ON (a.language_id = b.id) 
								WHERE a.member_id = '" . $row["id"] . "'"; 
		$result_lang 	= $db->query($query_lang);
		$row_langs 		= $db->rows($result_lang);
		$lang_str = '';
		foreach($row_langs as $row_lang) {
			$lang_str .= ($lang_str==''?'':';') . $words[strtolower($row_lang["title"])];
		}
		$html.= '<td ' . $width_two . '>' . cTYPE::gstr($lang_str) . '</td>';
	...
	$html.= '<tr>';
	$html.= '<td colspan="37" style="font-size:12px; font-weight:bold;">' . $words["total"] . ': ' . $cnt . '</td>';
	$html.= '</tr>';
