...
try {
		...
		if( $_SESSION[$_SERVER['HTTP_HOST'] . ".sysSessID"] == "" ) {
				...
				$result_sess = $sess_db->query("SELECT admin_id, session_id, platform FROM website_session WHERE deleted <> 1 AND session_id = '" . $sess_id . "'");
				if( $sess_db->row_nums($result_sess) > 0 )  {
					$row_sess = $sess_db->fetch($result_sess);
					$admin_id = $row_sess["admin_id"];
					$platform = $row_sess["platform"];
					$sess_db->query("UPDATE website_session SET last_updated = '" . time() . "' WHERE deleted <> 1 AND session_id = '" . $sess_id . "'");
					...
					if($admin_id == 999999) {
						...
						//normal user
						$result_user = $sess_db->query("SELECT * FROM website_admins WHERE deleted <> 1 AND status = 1 AND id = '" . $admin_id . "'");
