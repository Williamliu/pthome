...
try {
	...
	if(	$response["errorCode"] == 0 ) {
			$db = new cMYSQL($CFG["mysql"]["host"], $CFG["mysql"]["user"], $CFG["mysql"]["pwd"], $CFG["mysql"]["database"]);
				// normal user login
				$query = "SELECT id FROM website_admins WHERE deleted <> 1 AND status = 1 AND ( user_name = '" . $db->quote(trim($_REQUEST["login_name"])) . "' OR email = '" . $db->quote(trim($_REQUEST["login_name"])) . "') AND password = '" . $db->quote(trim($_REQUEST["login_pwd"])) . "'";
				...
				if( $db->row_nums($result) > 0 )  {
					...
					$db->query("UPDATE website_admins SET hits = hits + 1, login_count = 0, last_login = '". time() ."' WHERE deleted <> 1 AND status = 1 AND id = '" . $admin_id . "'");	
					...
					$query = "UPDATE website_admins SET login_count = login_count + 1 WHERE deleted <> 1  AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["login_name"])) . "' OR email = '" . trim($_REQUEST["login_name"]) . "')";
					$db->query( $query );
					
					$query = "SELECT id, login_count FROM website_admins WHERE deleted <> 1 AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["login_name"])) . "' OR email = '" . trim($_REQUEST["login_name"]) . "')";
					$result_count = $db->query( $query );
					$row_count =  $db->fetch($result_count);
					if($row_count["login_count"] >= 3) {
						$query = "UPDATE website_admins SET status = 0 WHERE deleted <> 1 AND status = 1 AND ( user_name = '" . cTYPE::utrans(trim($_REQUEST["login_name"])) . "' OR email = '" . trim($_REQUEST["login_name"]) . "')";
						$db->query( $query );
