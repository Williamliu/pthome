...
try {
	...
	$html.= '<tr>';
	$html.= '<td colspan="' . (13 + $hcnt) . '" align="center" height="40" style="font-size:12px; border:0px; font-weight:bold;">' . cTYPE::gstr($row0["title"]) . '<br>[ ' . date("M d, Y", $row0["start_date"]) . ($row0["start_date"]>0?' ~ ' .date("M d, Y", $row0["end_date"]):'') .  ' ]</td>';
	$html.= '</tr>';


	$html .= '<tr>';
	$html .= '<td colspan="' . (13 + $hcnt) . '" align="center"><span style="font-size:12px; font-weight:bold;">' . $words["event attendance"] . '</span></td>';
	$html .= '</tr>';
	...
	$query0 = "SELECT   a.id as enroll_id, a.event_id, a.group_no, a.shelf, a.trial, a.unauth, a.online, a.signin, a.graduate, a.cert, attend,
