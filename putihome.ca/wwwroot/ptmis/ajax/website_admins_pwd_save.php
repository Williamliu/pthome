...
try {
	...
	if( strlen($_REQUEST["password"]) < 6 || strlen($_REQUEST["repassword"]) < 6 ) {
		$response["errorCode"] 		= 1;
	 	$response["errorMessage"]	= "<br>密码必须是 6 位字符或以上,可以是数字或者英文字母.";
	...
	if(	$response["errorCode"] == 0 ) {
		...
		$query = "UPDATE website_admins SET login_count = 0,  password = '" . $_REQUEST["password"] . "', last_updated = '" . time() . "' WHERE deleted <> 1 AND id = '" . $_REQUEST["admin_id"] . "'";
		$db->query($query);
