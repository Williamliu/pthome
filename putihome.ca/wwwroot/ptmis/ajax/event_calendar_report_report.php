...
try {
	...
	$query_base = "SELECT  a.id as enroll_id, a.event_id, a.leader, a.volunteer, a.shelf, a.unauth, a.trial, a.group_no, 
	...
	while($row0 = $db->fetch($result0)) {
		...
		$query9 = "SELECT c.id, c.title, count(b.id) as count 
						...
						WHERE   a.start_date < '" . $evt_start_date . "' AND
								...
								b.member_id = '" . $row0["member_id"] . "'";
                                 
		$result9 	= $db->query($query9);
		$row9	    = $db->fetch($result9);
        $eObj["new"]		= ($row9["count"]>0?'':'Y');
