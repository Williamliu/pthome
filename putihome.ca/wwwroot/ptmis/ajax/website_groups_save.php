...
try {
	...
	if($_REQUEST["group_id"] < 0) {
		$query = "SELECT id FROM website_groups WHERE deleted <> 1 AND name = '" . cTYPE::utrans(trim($_REQUEST["group_name"])) . "'";
		...
		$query = "SELECT id FROM website_groups WHERE deleted <> 1 AND id <> '" . $_REQUEST["group_id"] . "' AND name = '" . cTYPE::utrans(trim($_REQUEST["group_name"])) . "'";
		$result = $db->query( $query );
		if( $db->row_nums($result) > 0 )  {
