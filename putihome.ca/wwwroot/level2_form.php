...
$query = "SELECT a.id, a.title, a.start_date, a.end_date, b.title as site_desc  FROM event_calendar a INNER JOIN puti_sites b ON (a.site = b.id) WHERE a.id = '" . $db->quote($_REQUEST["personalform_event_id"]) . "'";
...
$query 	= "SELECT * FROM puti_members WHERE deleted <> 1 AND status = 1 AND sess_id = '" . $db->quote($publicSession) . "'";
$result = $db->query($query);
$row 	= $db->fetch($result);
			  ...
			  $("#btn_submit").bind("click", function(ev) {
				  ...
				  $.ajax({
					  ...
					  error: function(xhr, tStatus, errorTh ) {
				  		  $("#wait").loadHide();
						  alert("Error (level2_form_save.php): " + xhr.responseText + "\nStatus: " + tStatus);
					  },
